...
| 功能 | 说明 |
| --- | --- |
| 📰 槽位化版面 | 添加槽位选角色（头条/正文/数据/快讯/提示框），固定版式按用户几何呈现，流式版式自动排布分页；角色不同边框线型不同（头条双线、数据虚线…） |
| ✋ 用户控版 | 默认固定版式：像 Word 调图片一样拖动槽位位置、拖角缩放；生成只填内容，不改页数、位置或尺寸；也可切换流式版式自动增高分页 |
...
| 📐 稳定适配 | 统一执行“生成 → 渲染实测 → 最多两轮定向改写 → 质检”；固定版式保持几何不变，流式版式按顺序增高分页；质量报告以实测为准 |
...
3. **预设** → 选一套模板 → 选择“固定版式/流式版式” → **生成** → AI 填充全部槽位（可勾选编辑部模式提升连贯性）
...
| 探针 | 覆盖 |
| --- | --- |
| `e2e.mjs` | 全链路：加载文档 → 生成/定向改写 → 真实测量与固定几何断言 → 质量检查 → 导出 PDF |
